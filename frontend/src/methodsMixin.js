...
export var methodsMixin = {
  methods: {
    ...
    focusEvent(i) {
      document.querySelectorAll(".custom-input")[i].style.borderBottom =
        "2px solid #1da1f2";
    ...
    loginUser() {
      if (
        this.$store.state.loginPasswordModel.trim() &&
        this.$store.state.loginUsernameOrEmailModel.trim()
      ) {
        http
          .post("/login", {
    ...
    editTweet() {
      if (!this.$store.state.newTweet.image) {
        return;
      }
      this.$store.state.isLoading = true;
      http
        .post("/editTweet", {
          tweetId: this.$store.state.editTweet._id,
          tweetImage: this.$store.state.newTweet.image,
        })
        .then((result) => {
          console.log("edited tweet ", result);
          this.$store.state.newTweet.text = "";
          this.$store.state.newTweet.image = "";
          this.$store.state.isLoading = false;
      ...
      http
        ...
        .then((result) => {
          ...
          if (add) {
            ...
            this.$store.state.currentUser.bookmarks.splice(
              this.$store.state.currentUser.bookmarks.indexOf(tweetId),
              1
            );
            this.$router.go(0);
    ...
    likeOrUnlike(tweetId, like) {
      ...
      if (like) {
        ...
        this.$store.state.currentUser.likedTweets.splice(
          this.$store.state.currentUser.likedTweets.indexOf(tweetId),
          1
        );
      }
    },
    ...
    getCurrentUser() {
      this.$store.state.isLoading = true;
      http
        .post("/getuserwithoutdetail", {
          username: localStorage.getItem("userId"),
        })
        .then(async (result) => {
          console.log("current user", result.data);
          this.$store.state.currentUser = await result.data;
          this.$store.state.newInfos.name =
            this.$store.state.currentUser.name || "";
          this.$store.state.newInfos.mail =
            this.$store.state.currentUser.mail || "";
          this.$store.state.newInfos.bio =
            this.$store.state.currentUser.bio || "";
          this.$store.state.newInfos.location =
            this.$store.state.currentUser.location || "";
          this.$store.state.newInfos.website =
    ...
    followOrUnfollow(follow) {
      ...
      if (follow) {
        this.$store.state.currentUser.following.push(
          this.$store.state.userForProfile._id
        );
        this.$store.state.userForProfile.followers.push(
          this.$store.state.currentUser._id
        );
      } else {
        this.$store.state.currentUser.following.splice(
          this.$store.state.currentUser.following.indexOf(
            this.$store.state.userForProfile._id
          ),
          1
        );
        this.$store.state.userForProfile.followers.splice(
          this.$store.state.userForProfile.followers.indexOf(
            this.$store.state.currentUser._id
          ),
          1
        );
    ...
    closeAddTweetPopup() {
      ...
      this.$store.state.editTweet = {};
    },
    openAddTweetPopup() {
      this.$store.state.addTweetPopup = true;
    },
    openEditTweetPopup() {
      this.$store.state.editTweetPopup = true;
    },
    closeEditTweetPopup() {
      this.$store.state.editTweetPopup = false;
    },
    routeToPath(p) {
