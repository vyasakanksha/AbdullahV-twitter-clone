<template>
  <div class="tweet-root">
    ...
    <div class="action-list3" v-if="actionList3">
      ...
      <div
        @click="addOrRemoveFromBookmarks(infoForTweet._id, false)"
        v-if="$store.state.currentUser.bookmarks.includes(infoForTweet._id)"
        class="tweet-actions-list-item"
      >
        <i class="far fa-bookmark"></i>
        Remove tweet from bookmarks
      </div>
      <div
        @click="addOrRemoveFromBookmarks(infoForTweet._id, true)"
        v-else
        class="tweet-actions-list-item"
      >
        <i class="far fa-bookmark"></i>
    ...
    <div class="tweet-section2" @click.stop="rootClick()">
      <div class="tweet-section2-header">
        <span class="header-sec1">
          <span
            @click.stop="
              $router.push({ path: '/' + infoForTweet.author.username })
            "
            class="name"
            >{{ infoForTweet.author.name }}</span
          >
          <span
            @click.stop="
              $router.push({
                path: '/' + infoForTweet.author.username,
              })
            "
            class="link"
            v-if="infoForTweet.author.username"
          >
            @{{ infoForTweet.author.username }}
          </span>
          <span class="uc">&#8226;</span>
          <span class="created-date">{{ infoForTweet.createdDateLocal }}</span>
          ...
          <span v-if="infoForTweet.parent">
            <span v-if="infoForTweet.parent.author">
              ...
              <span
                @click.stop="
                  $router.push({
                    path: '/' + infoForTweet.parent.author.username,
                  })
                "
                class="link"
                v-if="infoForTweet.parent.author.username"
              >
                @{{ infoForTweet.parent.author.username }}
              </span>
            </span>
        ...
        <span @click.stop="toggleTweetActionLVisible()" class="header-sec2"
          ><ActionButton
      ...
      <div class="tweet-content">
        <div class="tweet-text" v-if="infoForTweet.text">
          <pre
            v-html="infoForTweet.text"
            v-if="infoForTweet.isDetailed"
        ...
        <span v-if="infoForTweet.tweetImage">
          <img
            v-if="!infoForTweet.isDetailed && isImage()"
            @click.stop="makeZoomedImage()"
            class="tweet-img"
            :src="infoForTweet.tweetImage"
            alt=""
          />
          <img
            style="height: 500px"
      ...
      <div class="tweet-action-buttons">
        ...
        <span
          v-if="$store.state.currentUser.likedTweets.includes(infoForTweet._id)"
          @click.stop="unLikeTweet()"
          class="with-count"
        >
          <ActionButton
            iconClass="fas fa-heart"
            default-color="#e0245e"
            hover-color="#e0245e"
            hover-bg="#F5E1E7"
          ></ActionButton>
          <span class="count">{{ infoForTweet.likedUsers.length }}</span>
      ...
      <span class="f-and-f" v-if="infoForTweet.isDetailed">
        <span class="ff"
          ><b>{{ infoForTweet.retweeters.length }}</b>
          <span class="ftext">Retweet</span></span
        >
        <span style="padding: 0px 20px" class="ff"
          ><b>{{ infoForTweet.likedUsers.length }}</b>
          <span class="ftext">Likes</span></span
        >
      </span>
    </div>
  </div>
...
// import EditTwitterButton from "@/components/EditTwitterButton";
...
export default {
  ...
  mounted() {
    console.log("info", this.infoForTweet);
    if (this.infoForTweet.text) {
      var r = this.infoForTweet.text.replaceAll(
    ...
    const date = new Date(this.infoForTweet.createdDate);

    this.infoForTweet.createdDateLocal = date.toLocaleString();
  },
  methods: {
    // remtwe(){
    ...
    rootClick() {
      ...
      var redirectedPath =
        "/" +
        this.infoForTweet.author.username +
        "/tweets/" +
        this.infoForTweet._id;
    ...
    unLikeTweet() {
      ...
      this.infoForTweet.likedUsers.splice(
        this.infoForTweet.likedUsers.indexOf(this.$store.state.currentUser._id),
        1
      );
    },
    replieTweet() {
    ...
    editTweet() {
      this.$store.state.editTweet = this.infoForTweet;
      this.$store.state.addTweetPopup = true;
    },
  },
};
