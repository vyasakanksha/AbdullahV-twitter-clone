<template>
  <div class="profile-root">
    <div class="fixed-header-for-profile">
      <ActionButton
        action="goToBack"
        icon-class="fas fa-arrow-left"
        default-color="#1DA1F2"
        hover-color="#1DA1F2"
        hover-bg="#E8F5FE"
      ></ActionButton>
      <span>
        ...
        <h5 class="user-tweet-count">
          {{ $store.state.userForProfile.tweets.length }} tweet
        </h5>
      </span>
    </div>
    <img
      @click.stop="zoomedBannerImage()"
      class="banner-img"
      :src="bannerImage()"
      alt=""
    />
    <div class="profile-section">
      <div class="photo-and-buttons">
        ...
        >
          <TwitterButton
            v-if="
              $store.state.currentUser.following.includes(
                $store.state.userForProfile._id
              )
            "
            h="40px"
            w="40px"
      ...
      <h2>{{ $store.state.userForProfile.name }}</h2>
      <h4 class="user-tweet-count">
        @{{ $store.state.userForProfile.username }}
      </h4>

      <pre v-html="$store.state.userForProfile.bio"></pre>

      <span class="some-info" v-if="$store.state.userForProfile.location"
        ><i class="fas fa-map-marker-alt"></i
        ><span>{{ $store.state.userForProfile.location }}</span></span
      >
      <span class="some-info" v-if="$store.state.userForProfile.mail"
        ><i class="fas fa-envelope"></i
        ><a :href="'mailto:' + $store.state.userForProfile.mail">{{
          $store.state.userForProfile.mail
        }}</a></span
      >
      <span class="some-info" v-if="$store.state.userForProfile.website"
        ><i class="fas fa-link"></i
        ><a :href="$store.state.userForProfile.website">{{
          $store.state.userForProfile.website
        }}</a></span
      >
      <span class="some-info"
        ><i class="far fa-calendar-alt"></i
        ><span
          >Joined {{ $store.state.userForProfile.joinedDateLocal }}</span
        ></span
      >

      <span class="f-and-f">
        <span class="ff"
          ><b>{{ $store.state.userForProfile.following.length }}</b>
          <span class="ftext"> Following</span></span
        >
        <span class="ff"
          ><b>{{ $store.state.userForProfile.followers.length }}</b>
          <span class="ftext"> Followers</span></span
        >
      </span>
    ...
    <div style="position: relative; bottom: 95px">
      <div
        v-for="(tweet, index) in $store.state.userForProfile.tweets"
        :key="index"
      >
...
export default {
  ...
  methods: {
    ...
    bannerImage() {
      if (
        this.$store.state.userForProfile.bannerImage &&
        this.$store.state.userForProfile.bannerImage.match(this.urlRegex)
      ) {
    ...
    profilePhoto() {
      if (
        this.$store.state.userForProfile.profileImage &&
        this.$store.state.userForProfile.profileImage.match(this.urlRegex)
      ) {
  ...
  created() {
    ...
    this.$store.state.userForProfile.bio =
      //   this.$store.state.userForProfile.bio.replaceAll(
